...
# Unidecode::unidecode(mb_chr(0x5317).mb_chr(0x4EB0));
...
namespace Unidecode;
use Unidecode\UnidecodeError;


class Unidecode {

  private static function _get_table_path($table) {
    if (__FILE__ != "") {
  	  $script = __FILE__;
    }
    else {
  ...
  private static function _get_repl_str($char, $encoding=null) {
    ...
    if (!isset($GLOBALS["UNIDECODE_TABLE_$table"])) {
      $table_file = self :: _get_table_path($table);
  ...
  public static function unidecode($string, $errors=null, $replace_str=null, $encoding=null) {
    $retval = "";
    if (is_null($encoding))
    ...
    for ($index=0; $index < mb_strlen($string, $encoding); $index++) {
      ...
      $repl = self :: _get_repl_str($char, $encoding);
